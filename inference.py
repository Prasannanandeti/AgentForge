...
def run_inference():
    ...
    for tid in tasks:
        ...
        system_prompt = (
            ...
            "Reply with a JSON object only, keys: action_type, content. "
        ...
        while not done and step_idx < 8:
            ...
            try:
                ...
                if done:
                    success = "true"
                print(f"[STEP] step={step_idx} action={action.get('action_type','unknown')} reward={reward:.2f} done={str(done).lower()} error=null")
            except Exception as e:
                r = 0.05
                rewards.append(r)
                print(f"[STEP] step={step_idx} action=error reward={r:.2f} done=true error={str(e)}")
                done = True
